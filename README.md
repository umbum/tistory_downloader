...
optional arguments:
  ...
  -h2md       github page를 위한 옵션. html 내부의 article만 파싱해 .md 파일로 저장한다.
              또한 -o 옵션이 없을 경우 내부 이미지 데이터를 다운로드 하면서 img 태그 내부의 src 속성에 연결된 링크를 eg.github.io로 변경해서 저장한다.
  -b          eg.tistory.com의 html 파일을 저장한다. ( 백업용 )
```

* tistory에서 github page로 포스트를 옮기려면 -h2md 옵션을 주어 다운로드 한 다음, 생성된 데이터를 그대로 push하면 된다.
